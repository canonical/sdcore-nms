...
import {
  ...
  editDeviceGroup,
  deleteDeviceGroup,
} from "@/utils/deviceGroupOperations";
...
const DeviceGroupSchema = Yup.object().shape({
  ...
  networkSlice: Yup.string().min(1).required("Network slice is required."),
  ueIpPool: Yup.string()
    .required("IP pool is required")
    .test("is-cidr", "Invalid IP address pool.", (value) =>
      value ? validateCidr(value) : false,
    ),
  dns: Yup.string()
    .required("IP is required")
    .test("is-ip", "Invalid IP Address.", (value) =>
      value ? validateIp(value) : false,
    ),
  ...
  qosArp: Yup.number().min(1).max(15).required("ARP is required."),
});
...
export const DeviceGroupModal: React.FC<DeviceGroupModalProps> = ({
  ...
  const [networkSliceError, setNetworkSliceError] = useState<string | null>(
    null,
  );
    ...
    onSubmit: async (values) => {
      try {
        ...
        setTimeout(async () => {
          ...
          await queryClient.invalidateQueries({
            queryKey: [queryKeys.deviceGroupNames],
          });
          await queryClient.invalidateQueries({
  ...
  if (
    !isEdit &&
    !networkSlicesQuery.isLoading &&
    networkSliceItems.length === 0 &&
    !networkSliceError
  ) {
    setNetworkSliceError(
      "No network slice available. Please create a network slice.",
    );
  }
    ...
    >
      ...
      <Form>
        ...
        <Select
          ...
          onChange={(event) =>
            formik.setFieldValue("networkSlice", event.target.value)
          }
          options={[
            {
        ...
        <fieldset>
          ...
          <Select
            ...
            onChange={(event) =>
              formik.setFieldValue("qos5qi", Number(event.target.value))
            }
          ...
          <Select
            ...
            onChange={(event) =>
              formik.setFieldValue("qosArp", Number(event.target.value))
            }
...
export function CreateDeviceGroupModal({
  closeFn,
}: createNewDeviceGroupModalProps) {
  const auth = useAuth();
...
export function EditDeviceGroupModal({
  deviceGroup,
  closeFn,
}: editDeviceGroupActionModalProps) {
  const auth = useAuth();
  const handleSubmit = async (values: DeviceGroupFormValues) => {
    const MBRUpstreamBps = Number(values.MBRUpstreamMbps) * 1_000_000;
    const MBRDownstreamBps = Number(values.MBRDownstreamMbps) * 1_000_000;
  ...
  const qos5qiValue =
    deviceGroup["ip-domain-expanded"]?.["ue-dnn-qos"]?.["traffic-class"]?.qci;
  const initialQos5qi = valid5QIValues.includes(qos5qiValue) ? qos5qiValue : 0;
  const arpValue =
    deviceGroup["ip-domain-expanded"]?.["ue-dnn-qos"]?.["traffic-class"]?.arp;
  ...
  const initialValues: DeviceGroupFormValues = {
    ...
    MBRDownstreamMbps:
      deviceGroup["ip-domain-expanded"]?.["ue-dnn-qos"]?.["dnn-mbr-downlink"] /
        1_000_000 || null,
    MBRUpstreamMbps:
      deviceGroup["ip-domain-expanded"]?.["ue-dnn-qos"]?.["dnn-mbr-uplink"] /
        1_000_000 || null,
...
export const DeleteDeviceGroupButton: React.FC<
  deleteDeviceGroupActionModalProps
> = ({ deviceGroupName, subscribers }) => {
  const auth = useAuth();
  const queryClient = useQueryClient();
  const handleConfirmDelete = async (name: string) => {
    ...
    setTimeout(async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: [queryKeys.filteredSubscribers],
      });
    }, 100);
  };
  ...
  if (
    subscriberQuery.status == "error" &&
    is401UnauthorizedError(subscriberQuery.error)
  ) {
    auth.logout();
  }
