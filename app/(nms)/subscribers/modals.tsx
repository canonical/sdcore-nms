...
import {
  ...
  deleteSubscriber,
  editSubscriber,
} from "@/utils/subscriberOperations";
...
const SubscriberSchema = Yup.object().shape({
  ...
  networkSliceName: Yup.string().required(
    "Network slice selection is required.",
  ),
...
const SubscriberModal: React.FC<SubscriberModalProps> = ({
  ...
  const [selectedSlice, selectSlice] = useState<NetworkSlice | null>(
    previousSlice,
  );
  ...
  const deviceGroupNames =
    (deviceGroupQuery.data as string[]) || ([] as string[]);
  if (deviceGroupQuery.isError) {
    setDeviceGroupError("Failed to retrieve device groups.");
  }
  if (
    !deviceGroupError &&
    !deviceGroupQuery.isLoading &&
    deviceGroupNames.length === 0
  ) {
    setDeviceGroupError(
      "No device group available. Please create a device group.",
    );
  }

  const getFilteredDeviceGroups = (sliceName: string) => {
    const selectedSlice = networkSliceItems.find(
      (slice) => slice["slice-name"] === sliceName,
    );
    return (selectedSlice?.["site-device-group"] || []).filter((group) =>
  ...
  const handleSliceChangeCreate = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const filteredDeviceGroupOptions =
      getFilteredDeviceGroups(selectedSliceName);
    const selectedSlice = networkSliceItems.find(
      (slice) => slice["slice-name"] === selectedSliceName,
    );
    ...
    formik.setValues({
      ...
      deviceGroupName:
        filteredDeviceGroupOptions.length > 1
          ? ""
          : filteredDeviceGroupOptions[0] || "",
      plmnId: `${mcc}${mnc}`,
    });
  };
    ...
    >
      ...
      <Form>
        <Select
          ...
          error={
            formik.touched.networkSliceName
              ? formik.errors.networkSliceName
              : null
          }
          options={[
        ...
        <Select
          ...
          onChange={(event) =>
            formik.setFieldValue("deviceGroupName", event.target.value)
          }
          error={
            formik.touched.deviceGroupName
              ...
              ? deviceGroupNames
                  .filter((group) =>
                    (
                      ...
                      previousSlice?.["site-device-group"] ||
                      []
                    ).includes(group),
                  )
                  .map((deviceGroupName) => ({
              ...
              <Row className="p-form__control" style={{ display: "flex" }}>
                <Col size={2}>
                  <label
                    ...
                    style={{
                      color: isEdit || !selectedSlice ? "#999" : "inherit",
                    }}
                  >
                    {isEdit
                ...
                <Col size={isEdit ? 6 : 4}>
                  <Input
                    ...
                    error={
                      formik.touched.msin && formik.errors.msin
                        ? formik.errors.msin
                        : imsiError
                    }
                  />
        ...
        <fieldset>
            ...
            <div className="p-form__group p-form-validation row">
              <div className="u-align--right">
                <Button
                  appearance="positive"
                  type="button"
                  onClick={handleGenerateAuthValues}
                >
                  Generate
                </Button>
              </div>
          ...
          <Input
            ...
            error={
              formik.touched.sequenceNumber
                ? formik.errors.sequenceNumber
                : null
            }
          />
        </fieldset>
...
export const DeleteSubscriberButton: React.FC<deleteSubscriberButtonProps> = ({
  rawImsi,
}) => {
  const auth = useAuth();
  const queryClient = useQueryClient();
