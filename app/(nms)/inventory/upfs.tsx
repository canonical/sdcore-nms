...
export default function UpfTable() {
  ...
  if (query.status == "error") {
    ...
    return (
      <>
        <ErrorNotification error={"Failed to retrieve UPFs."} />
      </>
    );
  }
  ...
  const tableContent: MainTableRow[] = upfs.map((upf) => {
    return {
      key: upf.hostname,
      columns: [{ content: upf.hostname }, { content: upf.port }],
    };
  });
  ...
  return (
    <>
      {showNotification && (
        ...
        >
          To add UPFs to the inventory, integrate your UPF charm with the NMS
          charm. For more details, see the
          <a
      ...
      <MainTable
        ...
        headers={[{ content: "Hostname" }, { content: "Port" }]}
        rows={tableContent}
      />
    </>
  );
