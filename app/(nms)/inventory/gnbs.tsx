...
export default function GnbTable() {
  ...
  if (query.status == "error") {
    ...
    return (
      <>
        <ErrorNotification error={"Failed to retrieve gNodeBs."} />
      </>
    );
  }
  ...
  return (
    <>
      {showNotification && (
        ...
        >
          To add gNodeBs to the inventory, integrate your gNodeB charm with the
          NMS charm. For more details, see the
          <a
      ...
      {modalData?.action == EDIT && (
        <EditGnbModal gnb={modalData.gnb} closeFn={() => setModalData(null)} />
      )}
    </>
  );
}
