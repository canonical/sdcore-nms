...
async function apiGetSubscriber(
  rawImsi: string,
  token: string,
): Promise<Response> {
  if (!isValidSubscriberRawImsi(rawImsi)) {
    throw new OperationError(
      `Error getting subscriber: Invalid imsi provided ${rawImsi}.`,
    );
  }
  try {
...
async function apiPostSubscriber(
  rawImsi: string,
...
export async function deleteSubscriber(
  ...
  if (!isValidSubscriberRawImsi(rawImsi)) {
    throw new OperationError(
      `Error deleting subscriber: Invalid imsi provided ${rawImsi}.`,
    );
  }
  try {
    const response = await fetch(`/api/subscriber/imsi-${rawImsi}`, {
...
export async function getSubscribersTableData(
  token: string,
): Promise<SubscriberTableData[]> {
  try {
    ...
    const allSubscribers = await Promise.all(
      subscriberNames.map(async (subscriber: SubscriberId) => {
        const subscriberAuthData = await getSubscriberAuthData(
          subscriber.ueId,
          token,
        );
        const parents = findDeviceGroupByImsi(
          subscriber.ueId.split("-")[1],
          deviceGroups,
        );
        return {
          ...subscriberAuthData,
          networkSliceName: parents?.networkSliceName || "",
          deviceGroupName: parents?.deviceGroupName || "",
        };
      }),
    );
...
async function getSubscriberAuthData(
  ueId: string,
  token: string,
): Promise<SubscriberAuthData> {
  try {
    ...
    if (!response.ok) {
      throw new WebconsoleApiError(
        response.status,
        `Error retrieving subscriber data for Imsi ${rawImsi}`,
      );
    }
    ...
    if (
      !subscriberData ||
      !subscriberData.ueId ||
      !subscriberData.AuthenticationSubscription
    ) {
    ...
    const subscriberAuthData: SubscriberAuthData = {
      ...
      key:
        subscriberData.AuthenticationSubscription?.permanentKey
          ?.permanentKeyValue ?? "",
      sequenceNumber:
        subscriberData.AuthenticationSubscription?.sequenceNumber ?? "",
    };
    return subscriberAuthData;
  } catch (error) {
    console.error(
      `Error retrieving subscriber authentication data for ${ueId}: ${error}`,
    );
    throw error;
  }
}
...
export async function filterSubscribers(
  ...
  try {
    ...
    const existingImsis = new Set(
      allSubscribers.map((sub) => sub.ueId.split("-")[1]),
    );
...
async function doesSubscriberExist(
  rawImsi: string,
  token: string,
): Promise<boolean> {
  try {
  ...
  try {
    ...
    try {
      ...
      if (is409ConflictError(error)) {
        throw new OperationError(
          `Subscriber with IMSI ${subscriberData.rawImsi} already exists.`,
        );
      }
      throw error;
    }
    ...
    console.error(
      `Failed to create subscriber ${subscriberData.rawImsi} : ${error}`,
    );
    throw error;
  }
}
...
async function updatePreviousDeviceGroup(
  rawImsi: string,
  previousDeviceGroup: string,
  token: string,
): Promise<void> {
  try {
    ...
    if (is404NotFoundError(error)) {
      console.debug(
        `${previousDeviceGroup} not found when editing subscriber ${rawImsi}.`,
      );
      return;
    }
    console.error(
      `Failed to update device group ${previousDeviceGroup} : ${error}`,
    );
    throw error;
  }
}
